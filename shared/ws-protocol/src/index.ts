type WebSocketMessage<TType extends number, TData extends Record<TType, object>> = {
	[K in TType]: { type: K } & TData[K];
}[TType];

export class WebSocketProtocol<TType extends number, TData extends Record<TType, Object>> {
	public onOpen?: () => void;
	public onClose?: () => void;
	...
	public connect(url: string) {
		...
		websocket.onmessage = (event) => {
			const parsedData = JSON.parse(event.data) as WebSocketMessage<TType, TData>;
